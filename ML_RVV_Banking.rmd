...
Once categorical values have been transformed from character to factor, and partitions between training and validation have been created, let's look again how the values of those columns are distributed:
...
At this point it is important to visualize the data we have, so I will plot first of all numeric variables and second of all factor variables

Let's start with the histograms for numeric variables. This will allow us to get a first impression on those numeric variables distributiosn:
...
One possible proccess to apply to our data is "standarize" or scale continuous variables in order to have them all in the same range of values. "Standarize" a continuous normal variable X means to build a new variable $Z=\frac{X - \mu}{\sigma}$ where $\mu$ and $\sigma$ are the mean and the standard deviation of variable X. This new variable Z will follow a N(0,1) distribution.

In order to do so I have created a specific function called Standarize, but prior to applying it, it is necessary to know which of our numeric variables follow a normal distribution.

According to their histograms, plotted in Figure 1 to Figure 10 , most of our numeric variables do not follow a normal distribution, so the idea of standarizing may only apply to age.

Now it has been observed the aforementioned fact, let's try to see how the factor variables are distributed: 
...
Finally, let's analyze Pearson and Spearman correlation between numeric variables and dependant variable Y, to try to find out if all variables are needed for our candidate models or on the other hand if some variable can be dismissed:
