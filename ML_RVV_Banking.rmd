...
Once bankraw and validation were created, I split bankraw in train_set and test_set applying once again Pareto's law, and I will use theses train_set and test_set to train different models and test their performance over different metrics prior to decide which candidate model will be the final one.
...
In order to do so I have created a specific function called Standarize, but prior to applying it, it is necessary to know which of our numeric variables follow a normal distribution, so let's take a look at their histograms:
...
According to their histograms, most of our numeric variables do not follow a normal distribution, so the idea of standarizing may only apply to age.

Now it has been observed the aforementioned fact, let's try to see how the remaining variables are distributed with boxplots: 

```{r Exp_Analysis 3, message=FALSE}

colnames_fac<-colnames(train_set[idx_isc])

for(i in (1:length(colnames_fac))){
  print(    
    train_set%>%ggplot(aes(y,eval(parse(text=eval(colnames_fac[i]))))) +
    geom_boxplot() +
    ylab(colnames_fac[i]) +
