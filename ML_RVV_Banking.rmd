...
In this case I have tested 25 values ranging from 0 to 0,05 for compexity parameter value and left default minsplit value (at least 20) and default minbucket value obtaining following results:
...
## Save results to table
Results<-rbind(Results,tibble(method = "Random Forest", Accuracy = Acc_rpart, Balanced_Accuracy = Bacc_rpart,
...
After comparing results obtained by baseline model, logistic regression, k nearest neighbors, decision trees and  random forest decision trees are the most promising ones. Intuitively, I could expect better results from random forest but computational time taken did not help while optimizing hyperparameters.
